...
letter.values <- function(x, fiveNsum = T, number_lv = 5){
  ...
  new <- length(x)
  lower_ind <- NULL
  while(new != 1 & length(lower_ind) < number_lv){
    new <- (floor(new)+1)/2
  ...
  ans <- cbind(ans,lower_vals,middle,upper_vals,upper_vals-lower_vals)
  
  rownames(ans) <- letts
  colnames(ans) <- c("Depth","Lower","Middle","Upper","Spread")
